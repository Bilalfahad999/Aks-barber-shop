"use client";

import { motion } from "framer-motion";
import { Scissors, Mail, Phone, MapPin } from "lucide-react";

const hours = [
  { day: "Mon – Fri", time: "10AM – 8PM" },
  { day: "Saturday", time: "9AM – 8PM" },
  { day: "Sunday", time: "10AM – 6:30PM" },
];

function InstagramIcon() {
  return (
    <svg viewBox="0 0 24 24" width="16" height="16" fill="currentColor">
      <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zM12 0C8.741 0 8.333.014 7.053.072 2.695.272.273 2.69.073 7.052.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98C8.333 23.986 8.741 24 12 24c3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98C15.668.014 15.259 0 12 0zm0 5.838a6.162 6.162 0 100 12.324 6.162 6.162 0 000-12.324zM12 16a4 4 0 110-8 4 4 0 010 8zm6.406-11.845a1.44 1.44 0 100 2.881 1.44 1.44 0 000-2.881z" />
    </svg>
  );
}

function TikTokIcon() {
  return (
    <svg viewBox="0 0 24 24" width="16" height="16" fill="currentColor">
      <path d="M19.59 6.69a4.83 4.83 0 01-3.77-4.25V2h-3.45v13.67a2.89 2.89 0 01-2.88 2.5 2.89 2.89 0 01-2.89-2.89 2.89 2.89 0 012.89-2.89c.28 0 .54.04.79.1V9.01a6.34 6.34 0 00-.79-.05 6.34 6.34 0 00-6.34 6.34 6.34 6.34 0 006.34 6.34 6.34 6.34 0 006.33-6.34V8.69a8.28 8.28 0 004.84 1.56V6.78a4.85 4.85 0 01-1.07-.09z" />
    </svg>
  );
}

export default function Footer() {
  return (
    <footer className="bg-black border-t border-white/5 relative overflow-hidden">
      {/* Gold divider */}
      <div className="h-px bg-gradient-to-r from-transparent via-[#D4AF37]/50 to-transparent" />

      <div className="max-w-7xl mx-auto px-6 py-16">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">

          {/* Brand Column */}
          <div className="lg:col-span-1">
            <div className="flex items-center gap-3 mb-5">
              <div className="w-10 h-10 rounded-full border border-[#D4AF37]/50 flex items-center justify-center">
                <Scissors size={16} className="text-[#D4AF37]" />
              </div>
              <div>
                <span className="block font-serif text-xl font-bold gold-text">AK&apos;s</span>
                <span className="block text-[9px] text-[#C8C8C8] tracking-[0.35em] uppercase">Barber Shop</span>
              </div>
            </div>
            <p className="text-[#C8C8C8] text-sm leading-relaxed mb-6 max-w-xs font-sans">
              Premium grooming for the modern gentleman. Where precision meets artistry and luxury meets tradition.
            </p>

            {/* Real social links */}
            <div className="flex gap-3">
              <a
                href="https://www.instagram.com/akbarberz/?igshid=MzRlODBiNWFlZA%3D%3D"
                target="_blank"
                rel="noopener noreferrer"
                aria-label="Instagram"
                className="cursor-pointer w-9 h-9 rounded-full border border-white/10 flex items-center justify-center text-[#C8C8C8] hover:border-[#D4AF37] hover:text-[#D4AF37] hover:shadow-[0_0_12px_rgba(212,175,55,0.2)] transition-all duration-300"
              >
                <InstagramIcon />
              </a>
              <a
                href="https://www.tiktok.com/@aks_barbershop?_t=8gq3vyyqs7R&_r=1"
                target="_blank"
                rel="noopener noreferrer"
                aria-label="TikTok"
                className="cursor-pointer w-9 h-9 rounded-full border border-white/10 flex items-center justify-center text-[#C8C8C8] hover:border-[#D4AF37] hover:text-[#D4AF37] hover:shadow-[0_0_12px_rgba(212,175,55,0.2)] transition-all duration-300"
              >
                <TikTokIcon />
              </a>
            </div>
          </div>

          {/* Opening Hours */}
          <div>
            <h4 className="font-serif text-white font-semibold mb-5 text-sm">Opening Hours</h4>
            <div className="space-y-3">
              {hours.map((h) => (
                <div key={h.day} className="flex justify-between text-xs font-sans">
                  <span className="text-[#C8C8C8]">{h.day}</span>
                  <span className="text-[#D4AF37]">{h.time}</span>
                </div>
              ))}
            </div>
          </div>

          {/* Contact */}
          <div>
            <h4 className="font-serif text-white font-semibold mb-5 text-sm">Contact</h4>
            <ul className="space-y-4">
              <li className="flex items-start gap-3">
                <MapPin size={13} className="text-[#D4AF37] mt-0.5 flex-shrink-0" />
                <span className="text-[#C8C8C8] text-xs font-sans leading-relaxed">
                  1498 Dundas St, London, ON N5W 3B9<br />
                  1061 Wonderland Rd S Unit 5, London, ON N6K 3X4
                </span>
              </li>
              <li className="flex items-center gap-3">
                <Phone size={13} className="text-[#D4AF37] flex-shrink-0" />
                <a href="tel:+15194577777" className="text-[#C8C8C8] text-xs font-sans hover:text-[#D4AF37] transition-colors cursor-pointer">
                  +1 (519) 457-7777
                </a>
              </li>
              <li className="flex items-center gap-3">
                <Mail size={13} className="text-[#D4AF37] flex-shrink-0" />
                <a href="mailto:akbarbershop2@gmail.com" className="text-[#C8C8C8] text-xs font-sans hover:text-[#D4AF37] transition-colors cursor-pointer">
                  akbarbershop2@gmail.com
                </a>
              </li>
            </ul>
          </div>

        </div>
      </div>

      {/* Bottom bar */}
      <div className="border-t border-white/5">
        <div className="max-w-7xl mx-auto px-6 py-5 flex flex-col sm:flex-row items-center justify-between gap-3">
          <p className="text-[#C8C8C8]/50 text-xs font-sans">
            © {new Date().getFullYear()} AK&apos;s Barber Shop. All rights reserved.
          </p>
          <a
            href="/staff/login"
            className="text-[#C8C8C8]/25 hover:text-[#D4AF37]/60 text-xs font-sans transition-colors duration-300 cursor-pointer"
          >
            Staff Login
          </a>
        </div>
      </div>
    </footer>
  );
}
